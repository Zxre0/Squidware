...
void Client::DrawHUD() {
	...
	// colors
	const auto col_background = Color(40, 45, 50, 245);
	...
	std::string text = tfm::format(XOR("SquidWare | Build: " __DATE__ " | latency: %i ms"), ms);
	...
	if (g_menu.main.visuals.indicators.get(5) && g_visuals.m_c4_planted) {
		...
		//     https://github.com/ValveSoftware/source-sdk-2013/blob/master/sp/src/game/shared/sdk/sdk_gamerules.cpp#L173
		{
			...
			to_target = g_visuals.m_planted_c4_explosion_origin - dst;
			dist = to_target.length();

			// calculate the bomb damage based on our distance to the C4's explosion.
			range_damage = g_visuals.m_planted_c4_damage * std::exp((dist * dist) / ((g_visuals.m_planted_c4_radius_scaled * -2.f) * g_visuals.m_planted_c4_radius_scaled));
		...
		explode_time_diff = g_visuals.m_planted_c4_explode_time - g_csgo.m_globals->m_curtime;
		...
		damage_color = (final_damage < g_cl.m_local->m_iHealth()) ? g_menu.main.misc.menu_color.get() : colors::red;

		std::string bomb_text = tfm::format(XOR("bomb info | time: %i | damage: %i"), time_str, damage_str);
		render::FontSize_t bomb_size = render::hud.size(bomb_text);

		// background.
			//render::rect_filled(m_width - bomb_size.m_width - 20, 10 + (h_index * 24), bomb_size.m_width + 10, bomb_size.m_height + 2, { 24, 24, 24, 130 });
		render::gradient(m_width - bomb_size.m_width - 20, 10 + (h_index * 24), bomb_size.m_width + 10, bomb_size.m_height + 2, { 0, 0, 0, 0 }, { 24, 24, 24, 255 });

		// pand bar
			//render::rect_filled(m_width - bomb_size.m_width - 20, 28 + (h_index * 24), bomb_size.m_width + 10, 2, { color.r(), color.g(), color.b(), 255 });
		render::gradient(m_width - bomb_size.m_width - 20, 28 + (h_index * 24), bomb_size.m_width + 10, 2, { 0, 0, 0, 0 }, g_menu.main.misc.menu_color.get());

		// text.
		render::hud.string(m_width - 15, 10 + (h_index * 24), { 255, 255, 255, 255 }, bomb_text, render::ALIGN_RIGHT);
	...
	if (g_menu.main.visuals.indicators.get(4)) {
		int change = std::abs(math::NormalizedAngle(g_cl.m_body - g_cl.m_angle.y));

		std::string fake_text = XOR("lower-body");
		render::FontSize_t fake_size = render::hud.size(fake_text);

		// background.
		render::gradient(m_width - fake_size.m_width - 20, 10 + (h_index * 24), fake_size.m_width + 10, fake_size.m_height + 2, { 0, 0, 0, 0 }, { 24, 24, 24, 255 });

		// pand bar
		render::gradient(m_width - fake_size.m_width - 20, 28 + (h_index * 24), fake_size.m_width + 10, 2, { 0, 0, 0, 0 }, { 255 - change, 0 + change, 0, 255 });

		// text.
		render::hud.string(m_width - 15, 10 + (h_index * 24), { 255, 255, 255, 255 }, fake_text, render::ALIGN_RIGHT);
...
void Client::ClanTag()
{
	...
	if (g_menu.main.misc.clantag.get()) {
		...
		if (iPrevFrame != int(g_csgo.m_globals->m_curtime * 3.6) % 14) {
			switch (int(g_csgo.m_globals->m_curtime * 3.6) % 14) {
			case 0: {  SetClanTag(" "); break; }
			case 1: {  SetClanTag("S"); break; }
			case 2: {  SetClanTag("Sq"); break; }
			case 3: {  SetClanTag("Squ"); break; }
			case 4: {  SetClanTag("Squi"); break; }
			case 5: {  SetClanTag("Squid"); break; }
			case 6: {  SetClanTag("Squidw"); break; }
			case 7: {  SetClanTag("Squidwa"); break; }
			case 8: {  SetClanTag("Squidwar"); break; }
			case 9: {  SetClanTag("Squidware"); break; }
			case 10: { SetClanTag("Squidwar"); break; }
			case 11: { SetClanTag("Squidwa"); break; }
			case 12: { SetClanTag("Squidw"); break; }
			case 13: { SetClanTag("Squid");	break; }
			case 14: { SetClanTag("Squi");	break; }
			case 15: { SetClanTag("Squ");	break; }
			case 16: { SetClanTag("Sq");	break; }
			case 17: { SetClanTag("S");	break; }
			case 18: { SetClanTag(" ");	break; }

			default:;

			}
			iPrevFrame = int(g_csgo.m_globals->m_curtime * 3.6) % 28;
		}
...
void Client::print(const std::string text, ...) {
	...
	// print to console.
	g_csgo.m_cvar->ConsoleColorPrintf(colors::green, XOR("[Squidware] "));
...
void Client::UpdateRevolverCock() {
	...
	if (m_revolver_query == m_revolver_cock) {
		...
		// apply inattack.
		if (g_menu.main.misc.config.get() == 0 && m_revolver_query > m_revolver_cock)
